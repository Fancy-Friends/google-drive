...
npm run provider -- google-drive            # regenerate this repo
npm run provider -- google-drive --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/google-drive-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/google-drive-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/google-drive-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-google-drive` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/google-drive/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/google-drive` |
| npm | `@particle-academy/google-drive-ui`, `@particle-academy/google-drive-js` |
| Packagist | `particle-academy/google-drive-php` |
| PyPI | `fancy-google-drive` |
...
**Default to plain HTTP.** A Google Drive SDK is third-party code and is subject to
